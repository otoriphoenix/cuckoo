    ...
    if node["type"] == node_type:
        ...
        if current_val in attr_map:
            ...
            jsonpatch.apply_patch(
                json,
                [{"op": "add", "path": attr_path, "value": new_val}],
                in_place=True,
            )
...
def add_from_map(json, path, node_type, test_attr, new_attr, attr_map):
    node = resolve_pointer(json, path)
    if node["type"] == node_type:
        attr_path = path + test_attr
        current_val = resolve_pointer(json, attr_path)
        if current_val in attr_map:
            new_val = attr_map[current_val]
            jsonpatch.apply_patch(
                json,
                [{"op": "add", "path": path + new_attr, "value": new_val}],
                in_place=True,
            )
    ...
    for i, child in enumerate(node["content"]):
        add_from_map(
            json, path + f"/content/{i}", node_type, test_attr, new_attr, attr_map
        )
...
def replace_mentions(json, path, mention_type, mentions_map, alt_prefix):
    ...
    if node["type"] == "mention" and mention_type == node["attrs"]["type"]:
        ...
        if current_val in mentions_map.keys():
            ...
            jsonpatch.apply_patch(
                json,
                [{"op": "replace", "path": attr_path, "value": new_val}],
                in_place=True,
            )
        else:
                ...
                [
                    {
                        ...
                        "value": {
                            "type": "text",
                            "marks": [{"type": "link", "attrs": {"href": new_val}}],
                            "text": label,
                        },
                    }
                ],
    ...
    for i, child in enumerate(node["content"]):
        replace_mentions(
            json, path + f"/content/{i}", mention_type, mentions_map, alt_prefix
        )
