...
def json_endpoint(endpoint, json_data):
    ...
    while not answer_raw:
        answer_raw = requests.post(
            f"{os.getenv('OUTLINE_API')}/{endpoint}",
...
def file_endpoint(endpoint, metadata, file_data):
    answer_raw = requests.post(
        f"{os.getenv('OUTLINE_API')}/{endpoint}",
...
def import_html(filename, content, collection_id, parent_id=None):
...
def fetch_file(file_id):
    ...
    file = requests.get(
        f'{os.getenv("OUTLINE_API")}/fileOperations.redirect?id={file_id}',
